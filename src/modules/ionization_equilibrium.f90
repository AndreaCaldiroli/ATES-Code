	...
	use system_function           ! Equilibrium equations
	...
	integer, parameter :: N_eq = 3  ! Numbers of equations
	...
	! Column densities                                 
      ...
      real*8 :: sys_sol(3),sys_x(3),          & ! Equilibrium system setup
                tol,dpmpar,params(11),wa(lwa)
      real*8 :: start,finish                    ! Timing variables

      ! Predefined energy vectors      
	...
	ilo_q  =  ilo_f*(nhi(j)*s_hi(1)   +    &
               ...
               ! Upper boundary of i-th interval of integration (non-uniform geometric frequency grid)
               eup    = e_v(i)	

               ! Total optical depth at heigth r
		   deltal = (s_hi(i)*N1(j)                    &
		            +s_hei(i)*N15(j)                  &
               	      +s_heii(i)*N2(j))*1.0d-18	
      ...
      !$OMP CRITICAL
      heat(j)   = Hea_1*1.0d-18	
      P_HI(j)   = PIR_1*1.0d-18*erg2eV	
      P_HeI(j)  = PIR_15*1.0d-18*erg2eV
	P_HeII(j) = PIR_2*1.0d-18*erg2eV
	q(j)      = Hea_1/q_abs
	!$OMP END CRITICAL
	enddo
      ...
      do j = 1-Ng,N+Ng
	      ...
	      rcheiiiB(j) = rec_HeIII_B(TT)   ! HeIII
            ...
            coio =  2.179e-11*a_ion_HI(j)*nhi(j)  		   & ! HI
		      ...
		      + kb_erg*2.0*631515.0*a_ion_HeII(j)*nheii(j)   ! HeII
	...
	params(11) = a_ion_HeII(j)  
 	...
 	! Solve the system usein hybrd1 routine (from minpack)
      call hybrd1(sys_fcn,N_eq,sys_x,sys_sol,   &
